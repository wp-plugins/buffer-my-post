 ...
 */
function bmp_get_custom_posts($output)
{
 ...
 */
function bmp_get_post_taxonomies($custom_posts,$output)
{
 ...
 */
function bmp_get_post_labels($custom_posts,$output)
{
    //if there is more than one item than it's array of objects
    if(is_array($custom_posts))
    {
...
function bmp_get_category_taxonomies($post_taxonomies)
{
    //single records are objects(multi records are arrays)
    $count = bmp_countdim($post_taxonomies);
     if($count >1)
     {
...
function bmp_get_tag_taxonomies($post_taxonomies)
{
    foreach ($post_taxonomies as $name => $value)
    {
        foreach ($value as $val)
 ...
 */
function bmp_get_taxonomy_checklist($taxonomies, $omitCustCats)
{
...
function bmp_multi_implode($array, $glue) {
    $ret = '';
...
function bmp_is_assoc($array) {
    return (bool)count(array_filter(array_keys($array), 'is_string'));
}

//count Array dimensions
function bmp_countdim($array)
{
    if (is_array(reset($array)))
        $return = bmp_countdim(reset($array)) + 1;
    else
        $return = 1;
    return $return;
}